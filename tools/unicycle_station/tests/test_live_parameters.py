...
def test_schema_flags_and_mode_mask_both_required():
    ...
    frame=Frame('par',['1','lean_roll_kp','float','2.5','0','100','Lean','0.1','21'],'',time.monotonic())
...
def test_mock_live_writes_confirm_without_stopping_or_saving():
    ...
    try:
        e.start('mock');wait_until(lambda:len(e.params)==72)
        ...
        e.submit('apply',[('run_speed_straight',.3),('direction_heading_kp',1.0),('lean_roll_kp',3.5)],False)
        wait_until(lambda:len(e.batch_results)==3)
        assert all(ok for _,ok,_ in e.batch_results)
        assert e.params['lean_roll_kp'].value==3.5 and e.params['lean_roll_kp'].ram_dirty
...
def test_live_filter_and_same_screen_workbenches():
    ...
    try:
        ...
        for name,group,flags in [('lean_roll_kp','Lean','21'),('cam_exposure','Camera','4')]:
        ...
        assert not w.params.table.isRowHidden(w.params.rows['lean_roll_kp'])
        ...
        assert w.params.table.isRowHidden(w.params.rows['lean_roll_kp'])

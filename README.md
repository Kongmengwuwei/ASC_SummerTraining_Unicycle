...
| [参数参考](参数参考.md) | 从源码提取的全部 74 项参数、默认值、范围和菜单步长 |
...
| 参数 | 74 项运行参数、按名称及类型键保存、CRC 与逐字回读校验 |
...
当前压弯已改为持续转弯率方向项＋实际车速×转弯率前馈，参数、旧 Flash 兼容、验证与调节步骤见 [压弯控制说明](压弯控制说明.md)。压弯与 Roll 合成目标硬上限仍为 10°。此前算法及 9 月 9 日扩展上限的历史分析见 [转弯饱和与压弯评估](转弯饱和与压弯评估.md)，不作为当前公式说明。
